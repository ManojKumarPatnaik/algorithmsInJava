...
public class CriticalConnectionsInNetworkLeetocde1192 {
	...
	private void dfs(int node, int parent, ArrayList<ArrayList<Integer>> adjList, int[] dis, int[] minDis, int[] timer,
		...
		for (int adj : adjList.get(node)) {
			...
			/*
			 * If adjacent node can be discovered early than the current node, than update
			 * the minimum discovery time of current node as well
			 */
			minDis[node] = Math.min(minDis[node], minDis[adj]);
		}

		/*
		 * If current node can only be disovered from the parent node, than the parent
		 * to current node's edge is critical edge (bridge)
		 */
