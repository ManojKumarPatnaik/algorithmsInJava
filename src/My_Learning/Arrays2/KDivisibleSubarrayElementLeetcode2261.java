...
public class KDivisibleSubarrayElementLeetcode2261 {
	...
	public int countDistinct(int[] nums, int k, int p) {
		int n = nums.length;
		// we are storing hashcode for all the substrings so that we can compare them
		// faster.
		...
		for (int i = 0; i < n; i++) {
			...
			for (int j = i; j < n; j++) {
				hc = 199L * hc + nums[j]; // updating running hashcode, since we nums are <=200, we shall consider a
											// prime near 200 to avoid collision
				if (nums[j] % p == 0)
					cnt++;
