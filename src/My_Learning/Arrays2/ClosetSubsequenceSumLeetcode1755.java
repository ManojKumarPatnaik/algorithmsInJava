...
public class ClosetSubsequenceSumLeetcode1755 {
	...
	public int minAbsDifference(int[] nums, int goal) {
		...
		for (int secondSetSum : second) {
			...
			if (first.get(first.size() - 1) < left) { // all subset sums from first half are too small => Choose the
														// largest
			...
			if (pos >= 0) // Exact match found! => first.get(pos) + secondSetSum == goal
				return 0;
			else // If exact match not found, binarySearch in java returns (-(insertionPosition)
					// - 1)
				pos = -1 * (pos + 1);
			int low = pos - 1;
			ans = (int) Math.min(ans, Math.abs(secondSetSum + first.get(low) - goal)); // Checking for the floor value
																						// (largest sum < goal)
			ans = (int) Math.min(ans, Math.abs(secondSetSum + first.get(pos) - goal)); // Checking for the ceiling value
																						// (smallest sum > goal)
		}
		return ans;
	}
	...
	/**
	 * Generating all possible subset sums. 2 choices at each index,i -> pick vs do
	 * not pick
	 */
